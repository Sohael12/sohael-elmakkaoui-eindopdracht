...
import { Star, Play, Info, ChevronRight, Volume2, VolumeX } from "lucide-react"
...
const AnimeSection: React.FC<AnimeSectionProps> = ({ title, animes }) => {
    ...
    return (
        <motion.section
            ...
            className="w-full space-y-8 px-4 md:px-8 py-8"
        >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                >
                    <Button variant="ghost" className="text-[#FFD700] hover:bg-white/10 group transition-all duration-300">
                        View All
            ...
            {/* Anime grid */}
            <motion.div variants={containerVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pb-8">
                {animes.map((anime) => (
                    <motion.div
                        key={anime.id}
                        ...
                        className="h-full"
                    >
                        <Card className="h-full bg-black/40 backdrop-blur-lg border-[#FFD700]/20 hover:border-[#FFD700]/50 transition-all duration-300 overflow-hidden group relative">
                            {/* Glow effect on hover */}
                            <div className="absolute inset-0 bg-gradient-to-b from-[#FFD700]/0 to-[#FFD700]/0 group-hover:from-[#FFD700]/5 group-hover:to-[#FFD700]/0 transition-all duration-500"></div>
                            ...
                            <CardContent className="p-4 space-y-3 relative z-10">
                                <CardTitle className="text-xl font-bold text-white line-clamp-1 group-hover:text-[#FFD700] transition-colors duration-300">
                                    {anime.title}
                                </CardTitle>
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm text-white/80">{anime.rating?.toFixed(1) || "N/A"}</span>
                                    </div>

                                    {anime.genre && (
                                        <Badge variant="outline" className="bg-black/50 border-[#FFD700]/30 text-[#FFD700] text-xs">
                                            {anime.genre}
                                        </Badge>
                                    )}
                                </div>

                                {anime.description && <p className="text-xs text-white/60 line-clamp-2 mt-2">{anime.description}</p>}
                            </CardContent>

                            <CardFooter className="p-4 pt-0">
                                <Link href={`/anime/${anime.id}`} className="w-full">
                                    <Button className="w-full bg-gradient-to-r from-[#FFD700] to-[#FFA500] hover:from-[#FFA500] hover:to-[#FFD700] text-black font-bold transition-all duration-300 shadow-lg shadow-[#FFD700]/20 group-hover:shadow-[#FFD700]/40">
    ...
    return (
        ...
        >
            ...
            {!anime.highlightVideo && (
                <div className="absolute inset-0 bg-gradient-to-br from-[#FFD700]/20 to-[#FFA500]/20 flex items-center justify-center">
            ...
            {anime.genre && (
                <div className="absolute top-3 left-3 z-10">
                    <Badge className="bg-black/70 backdrop-blur-sm text-[#FFD700] border-[#FFD700]/30 text-xs font-medium">
                        {anime.genre}
                    </Badge>
                </div>
            )}
