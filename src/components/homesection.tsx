...
export default async function AllAnimesPage() {
    ...
    const enhancedAnimes = animesList.map((anime) => ({
        ...
        description:
            anime.description ||
            `Discover ${anime.title}, one of the best ${anime.genre || "anime"} series in our collection. Watch now and enjoy this amazing anime experience.`,
    }));
                    ...
                    <div>
                        {animesList.length > 0 ? (
                                    ...
                                    <Link key={anime.id} href={`/anime/${anime.id}`} className="transform transition-all hover:scale-105">
                                        <Card className="bg-[#1A1A2E] border-none rounded-xl overflow-hidden shadow-lg hover:shadow-purple-900/20">
                                            ...
                                            <CardContent className="p-4">
                                                ...
                                                <p className="text-xs text-gray-400 line-clamp-1">{anime.genre || "No genre"}</p>
                                            </CardContent>
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                ...
                                <h3 className="text-xl font-medium text-white mb-2">No animes found</h3>
                                <p className="text-gray-400 max-w-md">Currently, there are no animes available in our database.</p>
                            </div>
                        )}
                    </div>
                </section>
