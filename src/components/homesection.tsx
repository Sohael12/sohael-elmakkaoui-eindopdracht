                    ...
                    <div>
                        {animesList.length > 0 ? (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                                {animesList.map((anime) => (
                                    <Link key={anime.id} href={`/anime/${anime.id}`}
                                        ...
                                        <Card
                                            className="bg-[#1A1A2E] border-none rounded-xl overflow-hidden shadow-lg hover:shadow-purple-900/20">
                                            <CardHeader className="p-0">
                                                <div className="relative aspect-[2/3]">
                                                    ...
                                                    />
                                                    <div
                                                        className="absolute inset-0 bg-gradient-to-t from-[#1A1A2E] via-transparent to-transparent"></div>
                                                    <div
                                                        className="absolute top-2 right-2 bg-black/60 rounded-full px-2 py-1 flex items-center">
                                                        <Star className="text-yellow-400 fill-yellow-400 mr-1 h-3 w-3"/>
                                                        <span
                                                            className="text-xs font-medium">{anime.rating || "N/A"}</span>
                                                    </div>
                                                </div>
                                            </CardHeader>
                                            <CardContent className="p-4">
                                                <CardTitle
                                                    className="text-white text-base font-medium line-clamp-1 mb-1">
                                                    {anime.title}
                                                </CardTitle>
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                ...
                                <p className="text-gray-400 max-w-md">Er zijn momenteel geen animes beschikbaar in onze
                                    database.</p>
                            </div>
                        )}
                    </div>
